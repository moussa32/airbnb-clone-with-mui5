...
const CarouselCard = ({ location }) => {
  ...
  return (
    <Box className="carouselCard" sx={{ flexGrow: 1, position: "relative" }}>
      <Box
        sx={{ ...fixedIcon, backgroundColor: "transparent", border: "unset", cursor: "pointer" }}
        component="button"
        onClick={handleHeart}
      >
        {activeHeart ? <FaHeart size={24} color="#e04a3a" /> : <FaRegHeart size={24} color="#fff" />}
      </Box>
      {location.locationImages.length && (
        <SwipeableViews axis={"x"} index={activeStep} onChangeIndex={handleStepChange} enableMouseEvents>
          {location.locationImages.map((step, index) => (
            <div key={step.id}>
              <Box component="img" sx={carouselImage} src={step.url} alt={step.id}></Box>
            </div>
          ))}
        </SwipeableViews>
      )}
